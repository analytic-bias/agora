...
:::: {.aside}
::: {.opbullet}
- This symbolizes an optional content;
:::
::: {.wabullet}
- While this symbolizes an warning.
:::
::::
...
\end{aligned}
$$
[$\leftarrow$ area of the red cylinder at y]{#almar2 .aside .almar}
...
Sometimes,
$$
\sqrt{d x^2+d y^2}=\sqrt{1+\left(\frac{d y}{d x}\right)^2} d x
$$

$$
s=\int_a^b \sqrt{1+\left(\frac{d y}{d x}\right)^2} d x
$$
:::

## HW12.4 - SCalcCC5 1.6.010.
```{python}
#| label: fig-hw12-4-plot-1
#| fig-cap: "Plot of algebraic curve $C$ given by $x=t^2$ and $y=t^5$"
#| column: margin
#| echo: false
...
- You might (or might not) want to know that the singularity at $(0,0)$ is called a [cusp](https://en.wikipedia.org/wiki/Cusp_(singularity)).
:::
::::
