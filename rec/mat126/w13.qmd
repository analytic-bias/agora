...
## HW 10.7 - SCalcCC5 6.2.014.
```{python}
#| label: fig-hw10-7-plot-1
...
V&=\int_0^{1\class{alref}{\cssId{alref1}{}}} A(x)\,\mathrm{d}x\\
...
::: {#almar1 .callout-warning .column-margin .almar .compensate-above}
...
[The plot is interactive, and optionally you may also want to check the code by clicking on the triangle if you so wish. You may also use the Mathematica subscription from the university; GPT (preferably 4) and Copilot are good resources for coding questions.]{.aside}
```{python}
...
\end{aligned}
$$
[$\leftarrow\quad$ area of the red cylinder at y]{#almar2 .aside .almar}

[The red cylinder is extended for clarity.]{.aside}
```{python}
import plotly.graph_objects as go
import numpy as np
...
::: {.column-margin .callout-tip collapse="true"}
## Optional
...
f(x(t)) & = & g(y(t))\\
\leadsto\quad
x(t)=f^{-1}(t) &\text{and}& y(t)=g^{-1}(t)\class{alref}{\cssId{alref3}{}}\\\\\\
\end{array}
$$

::: {#almar3 .column-margin .almar .callout-tip .compensate-above collapse="true"}
## Optional
...
## Miscellanies (you probably don't need to read this)
